...
def send_verification(user):
    ...
    notify(user,"verify:"+uuid.uuid4().hex,"Verify your email","Verify your Mmemme Abia account: "+url,private=True)

def send_reset(user):
    token = default_token_generator.make_token(user)
    url = settings.FRONTEND_URL + "/reset-password?" + urlencode({"user":user.pk,"token":token})
    notify(user,"reset:"+uuid.uuid4().hex,"Reset your password","Reset your password: "+url,private=True)
