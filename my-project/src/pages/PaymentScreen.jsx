...
export default function PaymentScreen() {
  ...
  const handleConfirmFree = () => {
    navigate(`/Paymentsuccess?booking=${booking.id}`, {
      state: {
        event,
        tickets,
        attendee,
  ...
  if (!booking) return <p role="status">{loading ? 'Loading booking...' : error?.message}</p>;
  if (booking.status === 'CONFIRMED') return <Navigate to={`/Paymentsuccess?booking=${booking.id}`} replace />;
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <div className="text-sm text-gray-400 mb-4 flex gap-2">
            <span>{booking.kind === "EVENT" ? "Events" : "Bookings"}</span>
            &gt;
