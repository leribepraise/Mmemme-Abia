...
const HotelCard = ({ hotel }) => {
  const [isSaved, setIsSaved] = useState(() => {
    ...
    return savedHotels.some((savedHotel) => savedHotel.id === hotel.id);
  });

  const handleSave = (e) => {
    // Stop the heart click from opening the hotel page
    e.preventDefault();
    e.stopPropagation();
    ...
    if (isSaved) {
      ...
      // Save hotel
      const updatedHotels = [...savedHotels, hotel];

      sessionStorage.setItem("savedHotels", JSON.stringify(updatedHotels));
      setIsSaved(true);
    }

    // Tell SavedItems that the list changed
      ...
      <div className="bg-white rounded-[12px] border border-gray-200 overflow-hidden hover:shadow-md transition cursor-pointer">
        <div className="relative">
          ...
          <button
            type="button"
            onClick={handleSave}
            className="absolute top-3 right-3 bg-[#00000033] p-2 rounded-full"
          >
            <Heart
              className={`w-4 h-4 ${
