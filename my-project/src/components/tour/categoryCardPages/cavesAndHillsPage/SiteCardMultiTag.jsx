  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition">
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-gray-500 mt-2">{site.type}</p>

        <div className="flex gap-2 mt-2">
          {site.tags.map((tag) => (
            <span
              key={tag}
              className="bg-[#EAF4EB] text-[#3F783D] text-xs font-medium px-3 py-1 rounded-full"
            >
              {tag}
            </span>
          ))}
...
export default SiteCardMultiTag;
