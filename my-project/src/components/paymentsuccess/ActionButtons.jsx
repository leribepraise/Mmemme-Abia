...
const ActionButtons = ({ event, tickets, attendee, total, orderId, bookingId, booking }) => {
  ...
  const issued = issuedTickets.filter(ticket => ticket.booking === bookingId);
  ...
  return (
    <>
      ...
      <div ref={ticketRef} className="absolute -left-[9999px] top-0">
        {issued.map(ticket => <EventTicketPreview key={ticket.id}
          qrCode={ticket.qr_code}
          ref={null}
          event={event}
          tickets={[{ qty: 1 }]}
          attendee={attendee}
          total={Number(booking?.items.find(item => item.id === ticket.booking_item)?.unit_price || 0)}
          orderId={ticket.ticket_number}
        />)}
      </div>

      <div className="flex flex-col sm:flex-row gap-4 mt-10 w-full sm:w-auto">
        <NavLink to={booking?.kind === 'EVENT' ? `/ticket?booking=${bookingId}` : '/profile'}>
          <button className="bg-[#48782E] hover:bg-[#3a6125] text-white font-bold py-3 px-8 rounded-lg transition-colors text-sm shadow-sm w-full sm:w-auto">
            {booking?.kind === 'EVENT' ? 'View My Ticket' : 'My Bookings'}
          </button>
        </NavLink>

        <div className="relative w-full sm:w-auto">
          <button
            onClick={() => booking?.kind === 'EVENT' ? setOpen((prev) => !prev) : downloadJSON(`booking-${orderId}.json`, booking)}
          ...
          >
            {booking?.kind === 'EVENT' ? 'Download Ticket' : 'Download Confirmation'}
          </button>
      ...
      <button onClick={() => { window.location.href = '/'; }} className="mt-8 text-[#48782E] font-bold text-sm hover:underline">
        Back to Home
      </button>
    </>
  );
