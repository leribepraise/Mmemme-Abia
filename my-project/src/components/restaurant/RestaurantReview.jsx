...
const RestaurantReview = ({ vendor }) => {
  const reviews = vendor.customerReviews || [];
  ...
  return (
    <section>
      ...
      {reviews.length === 0 ? (
        <p className="text-[10px] text-gray-500">No reviews yet.</p>
      ) : (
          ...
          {reviews.map((review, i) => (
            <div key={i} className="border border-gray-200 rounded-lg p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-[10px] font-bold">{review.name}</h3>
